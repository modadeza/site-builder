...
function PartsListPanel({ parts, editing, setEditing, move, remove, globalColor }) {
  if (!parts.length) {
    return (
      <div style={{ padding: '40px 20px', textAlign: 'center', color: '#2a2a38', fontSize: 11, lineHeight: 1.7 }}>
        カタログからパーツを追加してください
      </div>
    )
  }
...
function EditorPanel({ editPart, editVals, editing, upd, globalColor }) {
  if (!editPart || !editVals) {
    return <div style={{ padding: '40px 20px', textAlign: 'center', color: '#2a2a38', fontSize: 11 }}>パーツを選択してください</div>
  }
  return (
...
function PreviewPane({ parts, iframeRef, device, setDevice }) {
          ...
          {DEVICES.map(d => (
            <button key={d.key} onClick={() => setDevice(d.key)}
              style={{
                ...
                borderRadius: 3, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 3, transition: 'all .15s',
              }}>
              <span>{d.icon}</span><span className="device-label">{d.label}</span>
            </button>
          ))}
        </div>
      </div>
      <div style={{
        flex: 1, background: parts.length ? '#1a1a22' : 'var(--bg)', overflow: 'auto',
        display: 'flex', justifyContent: 'center', alignItems: device === 'pc' ? 'stretch' : 'flex-start',
        padding: device === 'pc' ? 0 : '20px 0',
      }}>
        {parts.length ? (
          <div style={{
            width: currentDevice.width, maxWidth: '100%',
            height: device === 'pc' ? '100%' : 'auto', minHeight: device !== 'pc' ? '100%' : undefined,
            background: '#fff', boxShadow: device !== 'pc' ? '0 0 40px rgba(0,0,0,.4)' : 'none',
            borderRadius: device !== 'pc' ? '12px' : 0, overflow: device !== 'pc' ? 'hidden' : 'auto',
            ...
            <iframe ref={iframeRef}
              style={{ width: '100%', height: device === 'pc' ? '100%' : 'calc(100% - 28px)', minHeight: device !== 'pc' ? '600px' : undefined, border: 'none', display: 'block' }}
              title="preview" />
          </div>
        ) : (
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100vh', height: '100dvh' }}>
      ...
      <div className="mobile-tabs">
        {[
          { key: 'catalog', label: 'カタログ', icon: '📦' },
          { key: 'build', label: '構成', icon: '🔧', badge: parts.length },
          { key: 'preview', label: 'プレビュー', icon: '👁' },
        ].map(tab => (
          <button key={tab.key} className={mobileTab === tab.key ? 'active' : ''}
      ...
      <div className="mobile-layout">
        ...
        <div style={{ padding: '6px 12px', borderBottom: '1px solid var(--bdr)', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexShrink: 0 }}>
          <button onClick={() => setShowGlobalPanel(!showGlobalPanel)}
            style={{ padding: '4px 8px', background: showGlobalPanel ? '#2a1e3e' : 'var(--bg3)', color: showGlobalPanel ? 'var(--acc2)' : 'var(--dim)', border: '1px solid var(--bdr)', borderRadius: 4, fontSize: 9, fontWeight: 600, cursor: 'pointer' }}>
            🎨
          </button>
          <div style={{ display: 'flex', gap: 4 }}>
            <button onClick={handleDownloadZip} disabled={!parts.length}
              style={{ padding: '4px 10px', background: parts.length ? 'linear-gradient(135deg, var(--acc), var(--acc2))' : 'var(--bg3)', color: '#fff', border: 'none', borderRadius: 4, fontSize: 9, fontWeight: 600, cursor: parts.length ? 'pointer' : 'not-allowed', opacity: parts.length ? 1 : .4 }}>
              ↓ ZIP
            </button>
            <button onClick={handleDownload} disabled={!parts.length}
              style={{ padding: '4px 10px', background: 'var(--bg3)', color: 'var(--dim)', border: '1px solid var(--bdr)', borderRadius: 4, fontSize: 9, fontWeight: 600, cursor: parts.length ? 'pointer' : 'not-allowed', opacity: parts.length ? 1 : .4 }}>
              HTML
            </button>
          </div>
        </div>
        ...
        <div style={{ flex: 1, overflowY: 'auto' }}>
          ...
          {mobileTab === 'preview' && (
            <div style={{ height: '100%', display: 'flex', flexDirection: 'column' }}>
              <PreviewPane parts={parts} iframeRef={iframeRef} device={device} setDevice={setDevice} />
            </div>
          )}
        </div>
      </div>
        ...
        <div style={{ width: 320, borderRight: '1px solid var(--bdr)', display: 'flex', flexDirection: 'column', flexShrink: 0 }}>
          <div style={{ padding: '8px 10px', borderBottom: '1px solid var(--bdr)', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexShrink: 0 }}>
            ...
            <div style={{ display: 'flex', gap: 4 }}>
              ...
              <button onClick={handleDownload} disabled={!parts.length}
                style={{ padding: '4px 8px', background: 'var(--bg3)', color: parts.length ? 'var(--dim)' : 'var(--dim)', border: '1px solid var(--bdr)', borderRadius: 4, fontSize: 9, fontWeight: 600, cursor: parts.length ? 'pointer' : 'not-allowed', opacity: parts.length ? 1 : .4 }}>
                HTML
              </button>
            </div>
          </div>
          ...
          <div style={{ flex: 1, overflowY: 'auto', display: 'flex', flexDirection: 'column' }}>
            <div style={{ borderBottom: editing ? '1px solid var(--bdr)' : 'none' }}>
              <PartsListPanel {...listProps} />
            </div>
            <div style={{ flex: 1, overflowY: 'auto' }}>
